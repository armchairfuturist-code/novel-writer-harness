"""Hindsight canonical state store — persistent structured memory for novels.

Wires Hindsight (HTTP API, port 8888) as the canonical source of truth for:
...
Each chapter draft queries Hindsight for relevant state before writing,
then pushes updated state after writing.

Usage:
    from pipeline.hindsight_client import HindsightStore
    hs = HindsightStore(project_id="my-novel")
    hs.ensure_bank()
    char_state = hs.get_character_state("Maria")
    hs.record_chapter_complete(3, summary="...", word_count=4000)
"""
...
class HindsightStore:
    ...
    def __init__(
        self,
        project_id: str,
        base_url: str = HINDSIGHT_BASE,
        enabled: bool = True,
    ):
        self.project_id = project_id
        self.bank_id = f"{HINDSIGHT_BANK_PREFIX}{project_id}"
    ...
    def get_bank_stats(self) -> dict:
        """Get bank statistics."""
        resp = self._http.get(
            f"{self.base_url}/v1/default/banks/{self.bank_id}/stats"
        )
        if resp.status_code == 200:
            return resp.json()
        return {}
    ...
    def store_memory(
        ...
        """Store a memory/fact in the bank.

        Args:
            content: The factual statement (e.g., "Maria has blue eyes")
            tags: Category tags (e.g., ["character_trait", "appearance"])
            importance: 0.0 (trivial) to 1.0 (critical canonical fact)
    ...
    def recall(
        ...
        Returns:
            List of memory dicts with 'content', 'score', 'tags', 'metadata'
        """
        if not self.enabled:
            return []
    ...
    def ensure_bank_safe(self) -> bool:
        """Ensure bank exists, with error handling."""
        try:
    ...
    def get_character_traits(self, character_name: str) -> list[dict]:
        """Get all stored traits for a character."""
        return self.recall(
            f"character traits for {character_name}",
            k=10,
            tag_filter=["character_trait"],
        )

    def get_world_facts(self) -> list[dict]:
        """Get all stored worldbuilding facts."""
        return self.recall(
            "worldbuilding facts and lore",
            k=20,
            tag_filter=["world_fact"],
        )
    ...
    def get_active_threads(self) -> list[dict]:
        ...
        return self.recall(
            ...
            k=15,
            tag_filter=["plot_thread"],
            min_score=0.3,
        )

    def get_foreshadowing_debts(self) -> list[dict]:
        """Get foreshadowing obligations not yet paid off."""
        return self.recall(
            "unresolved foreshadowing obligations",
            k=15,
            tag_filter=["foreshadowing"],
            min_score=0.3,
        )

    def get_chapter_summaries(self) -> list[dict]:
        """Get canonical chapter summaries."""
        return self.recall(
            "chapter summary canonical",
            k=30,
            tag_filter=["chapter_summary"],
        )
    ...
    def record_character_trait(
        ...
        """Record a canonical character trait."""
        self.store_memory(
    ...
    def record_plot_thread(
        ...
        """Record a plot thread and its status.

        Args:
            thread_name: Short identifier for the thread
            status: 'active', 'resolved', 'abandoned'
            introduced_chapter: Chapter where thread was introduced
            resolved_chapter: Chapter where thread was resolved (if resolved)
        """
        status_text = {
    ...
    def record_foreshadowing(
        ...
        """Record a foreshadowing obligation."""
        payoff_str = (
    ...
    def mark_foreshadowing_paid(self, element: str, payoff_chapter: int):
        """Mark a foreshadowing element as paid off."""
        # Store a new memory overriding the old one
        self.store_memory(
    ...
    def record_chapter_summary(
        ...
        """Record canonical summary of a completed chapter."""
        self.store_memory(
    ...
    def format_context_for_drafting(self, chapter_num: int, chapter_summary: str) -> str:
        """Build a structured context string for the drafting prompt.

        Queries Hindsight for relevant character states, world facts,
        active threads, and foreshadowing debts. Formats into a
        human-readable context block.

        Args:
            chapter_num: Current chapter being drafted
            chapter_summary: This chapter's outline summary (used as retrieval query)

        Returns:
            str: Formatted context, or empty string if disabled/error
        """
        if not self.enabled:
        ...
        try:
            ...
            # Parallel queries for different state types
            char_facts = self.recall(
                f"character states before chapter {chapter_num}",
                k=8,
                tag_filter=["character_trait"],
            )
            world_facts = self.recall(
                f"world building relevant to {chapter_summary[:100]}",
                k=5,
                tag_filter=["world_fact"],
            )
            active_threads = self.recall(
                "unresolved plot threads active",
                k=5,
                tag_filter=["plot_thread"],
            )
            foreshadowing = self.recall(
                "unpaid foreshadowing obligations",
                k=5,
                tag_filter=["foreshadowing"],
            )
            prev_chapters = self.recall(
                f"recent chapter summaries before chapter {chapter_num}",
                k=3,
                tag_filter=["chapter_summary"],
            )

            parts = ["[CANONICAL STATE FROM HINDSIGHT]"]
            parts.append("")
    ...
    def _format_empty_context(self) -> str:
        return "[CANONICAL STATE: Hindsight unavailable or disabled]\n"
    ...
    def update_after_chapter(
        ...
        world_facts: Optional[list[tuple[str, str]]] = None,
        threads: Optional[list[tuple[str, str, int]]] = None,
        foreshadowing_elements: Optional[list[tuple[str, int]]] = None,
    ):
        """Update canonical state after a chapter is complete.

        This is the single entry point for post-chapter state sync.
        Call this after every chapter draft completes.

        Args:
            chapter_num: Completed chapter number
            title: Chapter title
            summary: Short summary of events
            pov: POV character
            word_count: Chapter word count
            key_events: List of key events in this chapter
            character_states: Dict of {char_name: {trait: value}} from this chapter
            world_facts: List of (location, fact) tuples revealed this chapter
            threads: List of (thread_name, status, resolved_chapter) tuples
            foreshadowing_elements: List of (element, expected_payoff_chapter) tuples
        """
        if not self.enabled:
            return

        try:
            # Record chapter summary (always)
            ...
            if threads:
                for thread_name, status, resolved_ch in threads:
                    self.record_plot_thread(
                        ...
                        resolved_chapter=resolved_ch if status == "resolved" else None,
                    )

            # Record foreshadowing plants
            ...
            # Fail gracefully — Hindsight is auxiliary, not critical path
            pass
    ...
    def scan_contradictions(
        ...
        """Check if a proposed trait value contradicts existing canonical state.

        Args:
            character_name: Character to check
            trait_type: Type of trait (e.g., 'eye_color', 'hair_color')
            new_value: Proposed value
            chapter_num: Chapter proposing this value

        Returns:
            List of conflicting memories
        """
        results = self.recall(
            f"{character_name} {trait_type} canonical state",
            k=5,
