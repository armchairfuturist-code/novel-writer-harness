...
6. **Debate** (opt-in) — three specialized LLM agents cross-examine each chapter against canonical state for continuity errors before revision
7. **Score and review** - mechanical quality checks (banned words, show-don't-tell) + AI literary critique
8. **Export** - full manuscript as markdown, plus PDF/epub if you have Pandoc installed
...
### Triadic Constraint Debate Protocol (SGDD) — opt-in with `--debate`
...
Three agents with different groundings cross-examine every chapter that scores below threshold:

| Agent | Grounding | What it checks | Model |
|---|---|---|---|
| **Lore Prosecutor** | `canonical_state.json` | Continuity breaks, trait drift, world-building violations, timeline errors | DeepSeek V4 Pro |
| **Plot Sentinel** | Foreshadow state machine + outline beats | Overdue threads, spontaneous plants, missed milestones, pacing violations | Kimi K2.6 |
| **Mechanical Magistrate** | Both transcripts + mechanical scores | Reconciles conflicts, deduplicates, outputs a single revision manifest JSON | Qwen3.5 9B |

The debate loop runs: parallel evaluation → cross-examination → magistrate verdict. If any FATAL continuity break is found, the chapter is forced into revision with the manifest as its revision prompt. Configurable via `config.debate.*` thresholds.
...
The foreshadowing engine and semantic embedding retrieval were built from scratch, inspired by patterns in MuMuAINovel's foreshadow tracking system. The debate protocol was inspired by multi-agent verification patterns in code review systems, adapted for narrative continuity.
